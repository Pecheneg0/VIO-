...
# -*- coding: utf-8 -*-
"""
Hybrid Visual-Inertial Flow Estimator (Vision + IMU Coast)
==========================================================
🔹 При наличии текстуры: Оптический поток + IMU-компенсация + KF обновление
🔹 При потере текстуры: KF прогнозирование (coast) + растущая ковариация + предупреждения
🔹 Всегда публикует скорость и одометрию (без "стоп-кадров")
"""
...
class HybridKalmanFilter:
    """1D KF с разделёнными методами predict (для IMU-коастинга) и update (для Vision)"""
    def __init__(self, R=0.5, Q=0.1):
        self.R = R  # Шум измерения (Vision)
    ...
    def predict(self, dt=1.0):
        """Шаг прогноза (используется при потере зрения)"""
        self.P += self.Q * dt
        # Ограничиваем P, чтобы ковариация не улетала в бесконечность
        self.P = min(self.P, 5.0)
        self.P *= max(0.0 1.0 - self.damping * dt) 
...
class HybridFlowNode:
    def __init__(self):
        ...
        self.max_coast_frames = rospy.get_param('~max_coast_frames', 60)  # ~2 сек при 30Hz
        ...
        #  HYBRID STATE
        self.kf_x = HybridKalmanFilter(R=0.7, Q=0.12)
        self.kf_y = HybridKalmanFilter(R=0.7, Q=0.12)
    ...
    def image_callback(self, msg):
        ...
        if curr_pts is not None and status is not None:
            ...
            if vision_ok:
                ...
                # 🔹 COAST MODE: Только прогноз KF (инерциальный дрейф)
                self.coast_counter += 1
                if self.coast_counter == 1:
                    rospy.logwarn("⚠️ Texture lost! Switching to IMU Coast mode (P growing).")
                self.vision_active = False
                
                # Ковариация растёт со временем без коррекции
                self.cov_xy = min(0.05 + self.coast_counter * 0.02, 2.0)
            ...
            # Если коастим слишком долго, обнуляем скорость для безопасности
