...
# -*- coding: utf-8 -*-
"""
Visual Flow Velocity Estimator (Production + IMU Compensation)
===============================================================
Рассчитывает линейную скорость дрона над землёй (X=вперёд, Y=вправо)
на основе оптического потока + высота лидара + точная IMU-компенсация вращения.
...
class FlowVelocityNode:
    def __init__(self):
        ...
        self.vel_deadband = rospy.get_param('~vel_deadband', 0.02)
        ...
        rospy.loginfo(f" Publishing: {self.output_topic} (X=forward, Y=right)")
        rospy.loginfo(" Ready.")
    ...
    def image_callback(self, msg):
        ...
        if curr_pts is not None and status is not None:
            ...
            if len(good_new) >= self.min_features:
                ...
                # 2.  ФИЗИЧЕСКИ КОРРЕКТНАЯ КОМПЕНСАЦИЯ ВРАЩЕНИЯ
                # При вращении камеры появляется ложный поток.
                # Для камеры, смотрящей вниз (стандарт PX4/ArduPilot):
                #   ωy (крен) влияет на поток по X
                #   ωx (тангаж) влияет на поток по Y
                imu_w = self._get_closest_imu(img_time)
                if imu_w is not None:
                    imu_vec = np.array(imu_w['w'])
                    imu_in_cam = self.R_cam_imu @ imu_vec

                    # Вычитаем вращательную компоненту из пиксельного потока
                    flow_x_comp = flow_x_px + imu_in_cam[1] * self.fx
                    flow_y_comp = flow_y_px - imu_in_cam[0] * self.fy
                    
                    # Отладка (первые 5 сек): раскомментируйте для проверки
                    # rospy.loginfo_throttle(1.0, f"IMU comp: ωx={imu_in_cam[0]:.2f}, ωy={imu_in_cam[1]:.2f} | flow corr: {flow_x_comp:.1f}, {flow_y_comp:.1f}")
                else:
