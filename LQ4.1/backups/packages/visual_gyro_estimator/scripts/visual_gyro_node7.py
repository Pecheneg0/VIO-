...
# -*- coding: utf-8 -*-
"""
Hybrid Visual-Inertial Flow Estimator (Production + TF + Tunable Parameters)
============================================================================
...
Всегда публикует данные + TF для 3D-визуализации (Foxglove/RViz).
"""

import rospy
import cv2
import numpy as np
import tf  #  Добавлено для публикации трансформаций
...
class HybridFlowNode:
    def __init__(self):
        ...
        # Читаем сырое значение из launch/rosparam
        raw_db = rospy.get_param('~vel_deadband', 0.015)
        # Ограничиваем диапазон: минимум 0.001 (чтобы не делить на 0), максимум 0.70
        self.vel_deadband = max(0.001, min(float(raw_db), 0.030))
        rospy.loginfo(f" vel_deadband clamped: {raw_db} → {self.vel_deadband:.3f} m/s")
        ...
        self.scale_factor = rospy.get_param('~scale_factor', 1.0)   # ручной калибр
        ...
        self.tf_br = tf.TransformBroadcaster()  #  Инициализация TF-броадкастера
        ...
        rospy.loginfo(f"KF: R={self.kf_R}, Q={self.kf_Q}, Damp={self.coast_damping}")
        rospy.loginfo(" Hybrid Node ready. TF & Odometry active.")
        ...
        if curr_pts is not None and status is not None:
            ...
            if vision_ok:
                ...
                if self.coast_counter == 1:
                    rospy.logwarn(" Texture lost! Switching to IMU Coast mode.")
            ...
            if self.lidar_received and not rospy.is_shutdown():
                ...
                if self.last_integ_time is None:
                    ...
                    if 0.01 < dt_pos < 0.15:
                        ...
                        odom.pose.pose.orientation.w = 1.0  # <-- БЫЛО 0.0
                        ...
                        #  ПУБЛИКАЦИЯ TF ДЛЯ 3D-ВИЗУАЛИЗАЦИИ
                        self.tf_br.sendTransform(
                            (self.pos_x, self.pos_y, self.current_height),
                        ...
                        rospy.loginfo_throttle(1.5, f" {mode_str} | X={self.pos_x:+.2f} Y={self.pos_y:+.2f} cov={c:.2f}")
